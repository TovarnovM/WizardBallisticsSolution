...
namespace MPAPI
{
    public sealed class Node : INode, IWorkerNode, IRegisteredNode, IDisposable
    {
        ...
        private void OpenAndConnectToRegistrationServer(string registrationServerNameOrAddress, int registrationServerPort, int listenerPort)
        {
            if (_host == null)
            {
                try
                {
                    ...
                    IPEndPoint registrationServerEndPoint = new IPEndPoint(GetIPV4Addr(Dns.GetHostEntry(registrationServerNameOrAddress).AddressList), registrationServerPort);
        ...
        IPAddress GetIPV4Addr(IPAddress[] addrs) {
            foreach (var ipa in addrs) {
