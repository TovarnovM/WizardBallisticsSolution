...
namespace Microsoft.Research.Oslo
{
    ...
    public static partial class Ode
    {
        ...
        #region Adams4 (Адамс-Башфорд)
        //[Obsolete("Fixed step RK45 method is provided only as an example")]        
        public static IEnumerable<SolPoint> Adams4(double t0,Vector x0,Func<double,Vector,Vector> f,double initialStep = 0.1d) {
            var opt = Options.Default;
            opt.InitialStep = initialStep;
            return Adams4(t0,x0,f,opt);
        }
        ...
        #region Runge-Kutta Dormand and Prince (RK547M)
        ...
        /// </example>
        /// <returns>Endless sequence of solution points</returns>
        public static IEnumerable<SolPoint> RK547M(double t0,Vector x0,Func<double,Vector,Vector> f,double step = 0) {
            var opt = Options.Default;
            opt.MaxStep = step;
            return RK547M(t0,x0,f,opt);
        }
